...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openCrudModal(type: 'school' | 'owner' | 'kiosk' | 'user', mode: 'create' | 'edit', data: any = {}) {
    ...
    if (type === 'school') {
      ...
      crudFields.innerHTML += wrap('Nombre Completo', `<input type="text" id="f_name" value="${data.name || ''}" placeholder="Ej: Juan Pérez" required>`);
      if (mode === 'create') {
        crudFields.innerHTML += wrap('Email', `<input type="email" id="f_email" value="" placeholder="juan@ejemplo.com" required>`);
        crudFields.innerHTML += wrap('Contraseña', `<input type="password" id="f_password" value="" placeholder="Mínimo 8 caracteres" minlength="8" required>`);
        const roleOptions = ['ADMIN', 'CASHIER', 'ENCARGADO', 'PARENT'].map(r =>
          `<label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" name="f_roles" value="${r}" ${r === 'CASHIER' ? 'checked' : ''} style="width:auto; padding:0; margin:0; outline:none; box-shadow:none;"> ${r}</label>`
        ).join('');
        crudFields.innerHTML += wrap('Roles', `<div style="display:flex; flex-wrap:wrap; gap:12px;">${roleOptions}</div>`);
        const optsS = allSchools.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
  ...
  crudForm.addEventListener('submit', async (e) => {
    ...
    if (type === 'school') {
       ...
       if (mode === 'create') body.apiKey = undefined; // API key is auto-generated by the backend
    } else if (type === 'user') {
       if (mode === 'create') {
         url = '/users';
         ...
         body = {
           ...
           email: (document.getElementById('f_email') as HTMLInputElement).value,
           password: (document.getElementById('f_password') as HTMLInputElement).value,
           roles: checkedRoles.length > 0 ? checkedRoles : ['CASHIER'],
           schoolId: parseInt((document.getElementById('f_schoolId') as HTMLSelectElement).value, 10),
         };
       } else {
         url = `/admin/users/${id}`;
    ...
    try {
      const result = await fetchApi<any>(url, { method: mode === 'create' ? 'POST' : 'PATCH', body: JSON.stringify(body) });
      ...
      if (type === 'kiosk' && mode === 'create' && result?.apiKey) showConfirm('¡Kiosco Agregado!', `La API Key asignada es: ${result.apiKey}\nGuárdala bajo llave.`, false);
    ...
    try {
      ...
      kiosks.forEach(k => {
        ...
        tr.innerHTML = `
          ...
          <td style="padding: 16px;"><code class="btn-copy-key" style="background:var(--bg-color); padding:4px 8px; border-radius:4px; cursor:pointer; font-size:0.8rem; word-break:break-all;" data-key="${k.apiKey}" title="Click para copiar">${k.apiKey}</code></td>
          <td style="padding: 16px; display: flex; gap: 8px; flex-wrap: wrap;">
            ...
            <button class="btn-rotate-key" style="background:#8b5cf6; color:white; padding:6px 12px; border-radius:8px; border:none; cursor:pointer;" data-id="${k.id}">Rotar Key</button>
      ...
      document.querySelectorAll('.btn-rotate-key').forEach(b => b.addEventListener('click', async (e) => {
        const id = (e.currentTarget as HTMLElement).getAttribute('data-id');
        if (await showConfirm('Rotar API Key', `¿Generar una nueva API Key para el kiosco #${id}? La anterior dejará de funcionar inmediatamente.`)) {
          try {
            const result = await fetchApi<any>(`/admin/kiosks/${id}/rotate-key`, { method: 'POST' });
            await showConfirm('Nueva API Key', `La nueva key es:\n${result.apiKey}\n\nActualizá la configuración del POS.`, false);
            loadKiosks();
      ...
      document.querySelectorAll('.btn-copy-key').forEach(b => b.addEventListener('click', async (e) => {
        const key = (e.currentTarget as HTMLElement).getAttribute('data-key') || '';
        await navigator.clipboard.writeText(key);
        showToast('Copiado', 'API Key copiada al portapapeles.', 'success');
      }));
    } catch(err:any) {}
  }
  ...
  async function loadUsers() {
    try {
      ...
      document.querySelectorAll('.btn-reset-pw').forEach(b => b.addEventListener('click', async (e) => {
        const id = (e.currentTarget as HTMLElement).getAttribute('data-id');
        if (await showConfirm('Forzar Reset de Contraseña', '¿Generar un link de restablecimiento para este usuario? El usuario deberá elegir su nueva contraseña.', false)) {
          try {
            const result = await fetchApi<any>(`/admin/users/${id}/reset-password`, { method: 'POST' });
            // Show the reset URL to the admin so they can share it
            const resetUrl = result.resetUrl || '';
            await navigator.clipboard.writeText(resetUrl);
            showToast('Link Generado', `Link de restablecimiento copiado al portapapeles. Válido por ${result.expiresIn}.`, 'success');
          } catch (err: any) {
            showToast('Error', err.message, 'error');
          }
        }
